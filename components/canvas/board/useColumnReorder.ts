...
export function useColumnReorder({
  ...
  const flushPendingColumnUpdates = () => {
    if (pendingUpdateRef.current) {
      pendingUpdateRef.current();
    }
  };

  const flushRef = useRef(flushPendingColumnUpdates);
  useEffect(() => {
    flushRef.current = flushPendingColumnUpdates;
  });
  ...
  const handleColumnDrop = (event: DragEndEvent): boolean => {
