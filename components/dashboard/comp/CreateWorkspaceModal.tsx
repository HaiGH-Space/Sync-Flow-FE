    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogTrigger asChild>
        <Button
          variant="outline"
          size="icon"
          className="size-10rounded-xl cursor-pointer"
...
function CreateWorkspaceDialog(props: { onClose(open: boolean): void }) {
  ...
  return (
    <DialogContent>
      <DialogHeader>
        ...
        <DialogDescription>
          {tDashboard("workspace.create.description")}
        </DialogDescription>
      </DialogHeader>
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <div className="space-y-2">
          <Label htmlFor="workspace-name">
            {tDashboard("workspace.create.nameLabel")}
          </Label>
          <Input
            id="workspace-name"
            value={workspaceName}
