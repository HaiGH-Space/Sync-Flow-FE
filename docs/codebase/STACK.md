...
| ---------------------------- | ---------- | ------------------------------------ | ------------------------------------------------------------------------------------ |
| `livekit-client`             | `^2.21.0`  | WebRTC video/audio client SDK        | `package.json`, `hooks/use-video-call.ts`, `lib/api/video.ts`                         |
...
| `socket.io-client`           | `^4.8.1`   | Realtime chat & notifications socket | `package.json`, `lib/api/chat.ts`, `lib/api/notification.ts`                         |
| `zustand`                    | `^5.0.11`  | Client UI state store                | `package.json`, `lib/store/use-dashboard.ts`, `lib/store/use-call-store.ts`          |
...
pnpm test     # Runs Vitest unit and integration test suite (22 test files, 83 tests)
...
- Required env vars:
  - `NEXT_PUBLIC_API_URL`: Backend API endpoint for client requests and rewrite mapping (defaults to `http://127.0.0.1:8000`).
  - `INTERNAL_API_URL`: Optional direct backend URL for server-side rendering (SSR) in Docker / internal networks.
  - `VERCEL_URL`: Provided by Vercel deployment environments when applicable.
